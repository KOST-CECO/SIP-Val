		...
		implements Validation1cNamingModule
{
	...
	public boolean validate( File sipDatei ) throws Validation1cNamingException
	{
		...
		// eine Reihe *.xsd files befinden wie definiert
		// im Konfigurationsfile.

		List<String> allowedXsd = getConfigurationService()
				.getAllowedXsdFileNames();
		...
		for ( Iterator<String> iterator = allowedXsd.iterator(); iterator
				.hasNext(); ) {
			String xsdfilename = iterator.next();
			allowedXsdFiles.put( "header/xsd/" + xsdfilename, "header/xsd/"
					+ xsdfilename );
		}
