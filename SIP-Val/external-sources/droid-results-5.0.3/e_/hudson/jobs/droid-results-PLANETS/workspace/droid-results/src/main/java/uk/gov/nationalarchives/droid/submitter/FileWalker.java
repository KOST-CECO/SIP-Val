...
public class FileWalker extends DirectoryWalker {
    ...
    protected boolean handleDirectory(File dir, int depth, Collection results) {
        ...
        if (fastForward) {
            if (!(depth < recoveryRoad.size() && recoveryRoad.get(depth).getUri().equals(dir.toURI()))) {
                // This directory is NOT on our road to recovery.
                if (recoveryRoad.get(depth - 1).containsChild(dir.toURI())) {
    ...
    public static final class ProgressEntry {
         ...
         */
        ProgressEntry(URI uri, long id, List<URI> children) {
            this.uri = uri;
            this.id = id;
