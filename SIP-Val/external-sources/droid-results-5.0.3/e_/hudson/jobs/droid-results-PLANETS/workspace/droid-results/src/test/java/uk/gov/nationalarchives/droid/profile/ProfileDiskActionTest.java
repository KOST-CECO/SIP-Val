...
public class ProfileDiskActionTest {
    ...
    public void testSaveProfileToFile() throws Exception {
        ...
        assertEquals("db/file2", FilenameUtils.separatorsToUnix(entries.nextElement().getName()));
        assertEquals("db/file3", FilenameUtils.separatorsToUnix(entries.nextElement().getName()));
        assertEquals("db/file4", FilenameUtils.separatorsToUnix(entries.nextElement().getName()));
        assertEquals("db/service.properties", FilenameUtils.separatorsToUnix(entries.nextElement().getName()));
        assertEquals("profile.xml", FilenameUtils.separatorsToUnix(entries.nextElement().getName()));
        ...
        verify(callback, atLeastOnce()).onProgress(argThat(Matchers.lessThan(100)));
