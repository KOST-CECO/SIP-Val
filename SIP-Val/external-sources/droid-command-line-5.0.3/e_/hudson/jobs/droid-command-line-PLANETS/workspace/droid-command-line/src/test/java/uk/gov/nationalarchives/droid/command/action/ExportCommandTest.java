...
public class ExportCommandTest {
    ...
    public void testExportThreeProfiles() throws Exception {
        ...
        when(exportManager.exportProfiles(any(List.class), eq("destination"), (Filter) isNull())).thenReturn(future);
        ...
        verify(exportManager).exportProfiles(Arrays.asList(expectedExportedProfiles), "destination", null);
    ...
    public void testExportProfileWithNarrowFilter() throws Exception {
        ...
        Future future = mock(Future.class);
        when(exportManager.exportProfiles(any(List.class), eq("destination"), any(Filter.class))).thenReturn(future);
        ...
        verify(exportManager).exportProfiles(eq(Arrays.asList(expectedExportedProfiles)), 
                eq("destination"), filterCaptor.capture());
