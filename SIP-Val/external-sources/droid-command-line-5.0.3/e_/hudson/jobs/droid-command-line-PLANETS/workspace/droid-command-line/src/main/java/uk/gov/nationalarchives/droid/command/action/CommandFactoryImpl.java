...
public class CommandFactoryImpl implements CommandFactory {
     ...
     */
    public DroidCommand getExportCommand(CommandLine cli) throws CommandLineSyntaxException {
        
        if (!cli.hasOption(CommandLineParam.PROFILES.toString())) {
            throw new CommandLineSyntaxException("No profiles specified for export.");
        }
        
        String destination = cli.getOptionValue(CommandLineParam.EXPORT.toString());
        
        String[] profiles = cli.getOptionValues(CommandLineParam.PROFILES.toString());
        ExportCommand cmd = context.getExportCommand();
    ...
    public DroidCommand getReportCommand(CommandLine cli) throws CommandLineSyntaxException {
        ...
        if (!"planets".equalsIgnoreCase(reportType)) {
            throw new CommandLineSyntaxException(reportType
                    + " report is not a valid report type.");
        }
