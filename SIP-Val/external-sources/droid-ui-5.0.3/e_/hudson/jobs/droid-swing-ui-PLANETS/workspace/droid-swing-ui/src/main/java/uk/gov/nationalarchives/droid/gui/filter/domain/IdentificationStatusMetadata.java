...
public class IdentificationStatusMetadata extends GenericMetadata {

    /**
     * Default Constructor.
     */
    public IdentificationStatusMetadata() {
        ...
        for (NodeStatus nodeStatus : NodeStatus.values()) {
            addPossibleValue(new FilterValue(index++, nodeStatus.getStatus(),
                    String.valueOf(nodeStatus.getId())));
        }
    }
    ...
    public void validate(String stringTovalidate) throws FilterValidationException {
        if (StringUtils.isBlank(stringTovalidate)) {
            throw new FilterValidationException("Identification status can not be blank");
        }
    }
}
