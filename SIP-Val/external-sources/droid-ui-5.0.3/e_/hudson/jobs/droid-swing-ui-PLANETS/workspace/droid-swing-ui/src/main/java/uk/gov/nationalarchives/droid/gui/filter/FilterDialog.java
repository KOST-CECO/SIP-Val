...
import uk.gov.nationalarchives.droid.gui.filter.action.ApplyFilterAction;
...
public class FilterDialog extends JDialog {
     ...
     *            Profile Manager.
     */
    public FilterDialog(Frame parent, boolean modal, FilterImpl filterContext, DroidUIContext droidContext,
    ...
    private boolean isFilterPrevioudlyLoaded() {
        return filterContext.getFilterCriteriaMap().size() > 0;
    }
        ...
        operationCombobox.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                ...
                if (e.getStateChange() == ItemEvent.SELECTED && !filterPredicatesLoading) {
                    ...
                    FilterCriterionImpl filterCriteria = filterContext.getFilterCriteria(filterTable.getSelectedRow());
                    JComboBox sourceCombo = (JComboBox) e.getSource();
                    filterCriteria.setOperator((CriterionOperator) sourceCombo.getSelectedItem());
                }
            }
        });
        ...
        jButton2.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (tableModel.getRowCount() > 1 && (filterTable.getSelectedRow() + 1) != tableModel.getRowCount()) {
                    int size = filterContext.getFilterCriteriaMap().size();

                    filterContext.removeFilterCiteria(filterTable.getSelectedRow());
                    ...
                    // remove the last one
                    filterContext.removeFilterCiteria(size - 1);
                ...
                if (selectedItem != null) {
                    ...
                    if (e.getStateChange() == ItemEvent.SELECTED) {
                        ...
                        if (comp instanceof TextBoxAndButton) {
                            ((TextBoxAndButton) comp).setType(metadata, filterContext.getFilterCriteria(filterTable
                                    .getSelectedRow()));
                        }
                        filterTable.repaint();
                    }
