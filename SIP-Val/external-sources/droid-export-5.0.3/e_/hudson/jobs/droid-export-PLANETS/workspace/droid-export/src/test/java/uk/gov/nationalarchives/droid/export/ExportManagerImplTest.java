...
public class ExportManagerImplTest {
    ...
    public void testExportNodesToCsv() throws Exception {
        ...
        final List<ProfileResourceNode> nodes = new ArrayList<ProfileResourceNode>();
        nodes.add(node1);
        nodes.add(node2);
        nodes.add(node3);
        ...
        exportManager.exportProfiles(profileIdList, "destination", null).get();

        verify(writer).open(any(Writer.class));
        verify(writer).write(nodes);
        verify(writer).close();
    }
